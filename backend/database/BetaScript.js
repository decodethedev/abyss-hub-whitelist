const Script = require("../models/BetaScript");
const fs = require('fs');
const path = require("path");
const { getRandomString } = require("../security/random");

async function getBetaScriptBy(field, value) {
...
async function getBetaScriptByPlaceID(placeID) {
...
async function updateBetaScript(filter, update) {   
...
async function updateBetaScriptString(filter, update) {   
...
async function getAllBetaScripts() {
    const scripts = await Script.find();
    if (!scripts) {
        return null;
    }
...
async function deleteBetaScriptByID(script_id) {
...
async function createBetaScript(
    script_name,
    uploaded_by,
    primary_place_id,
    script_str,
...
module.exports = {getAllBetaScripts, deleteBetaScriptByID, updateBetaScriptString ,createBetaScript, getBetaScriptBy, updateBetaScript, getBetaScriptByPlaceID};